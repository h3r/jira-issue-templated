...
var JiraIssueSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  displayNoteTemplateSettings() {
    ...
    //template picker
    new import_obsidian2.Setting(containerEl).setName("Note template").setDesc("Template to use when creating a new note from a Jira issue.").addText((text) => 
    text
    .setPlaceholder("_templates/jira-issue-template.md")
    ...
    //folder picker where we want to save the new note
    new import_obsidian2.Setting(containerEl).setName("Note folder").setDesc("Folder where to save the new note.").addText((text) => 
      text
      .setPlaceholder("Example: Jira issues")
        ...
        if (connectedNotes.length > 0) {
          ...
          if (SettingsData.noteTemplate) {
            ...
            el.addEventListener("click", async () => {
              ...
              const folder = SettingsData.noteFolderPath ?? "";
              ...
              //create note
              try {
                let newNote = await app.vault.create(fullPath, templateContents)
                newNote.edit();
              } catch (error) {
                console.debug("Error writing template",error)
                return("Couldn't create file '" + fullPath + "': " + error.toString() )
              }   
